...
2. 再添加一个DMA模块，将两个DMA的Buffer Length改为23，Memory Map
   DataWidth修改为64，提高带宽。两个模块配置如下：
...
由于FCLK_CLK2只能引出一个引脚，需要在空白处右键选择create
port，填写引脚名称，方向，类型，并将引脚连接到FCLK_CLK2
...
8. 连接好其余信号，保存，点开Address
   Editor，查看地址配置，如果有些模块没有配置地址，点击Auto Assign
   Address
...
9. Generate Output Products和Create HDL
   Wrapper，在XDC中绑定AD9238引脚，之后生成bit文件
...
2. 设置ADC_COE为16，
   2^4，也就是将AD9238的12bit转换成8bit，ADC_BYTE为2，ADC_BITS为有效数据宽度，设置为12，FPGA程序中将12bit数据扩展为16bit。
